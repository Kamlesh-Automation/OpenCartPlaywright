...
async function performRegistration(page: Page): Promise<string> {
    ...
    await homePage.clickOnMyAccount();       // Click "My Account" link
    await homePage.clickOnRegister();        // Click "Register" option
    ...
    await registrationPage.checkPrivacyPolicy();  // Accept the privacy policy
...
async function addProductToCart(page: Page) {
    ...
    await productPage?.setQuantiy(productQuantity);
...
async function verifyShoppingCart(page: Page) {
    ...
    const shoppingCartPage: ShoppingCartPage = await productPage.clickonViewCartButton();
