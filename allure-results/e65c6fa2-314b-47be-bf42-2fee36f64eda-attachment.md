...
```ts
  ...
  55 |     expect(await searchResultPage.isProductExist(config.productName)).toBeTruthy();
  56 | 
  57 |     //Step:6-7-8 select product ->. set quantity -> Add to cart -> Verify confirmation
  58 |     if (await searchResultPage.isProductExist(config.productName)) 
  59 |     {
  60 |         await searchResultPage.selectProduct(config.productName);
  61 |         await productPage.setQuantiy(config.productQuantity);
